...
export interface GameEndSceneConfig {
  displayImage: string; // webpack base64 import for background
}
...
export class GameEndScene implements Scene {
  ...
  async enter(): Promise<void> {
    // Full-screen tap target — tapping anywhere triggers CTA (AppLovin requirement)
    ...
    // Background image — full screen, scaled to cover
    ...
    // Logo — shown instead of bg in landscape
  ...
  private drawHitArea(): void {
    ...
    this.hitArea.clear();
    if (isLandscape) {
      // Solid dark background in landscape (no splash image)
  ...
  // Stored layout position for animation
  private buttonY = 0;
  ...
  private layoutScene(): void {
    ...
    // Background — scale to cover, anchored to top (portrait only)
    ...
    // Logo — centered above button (landscape only)
